 ...
 */
function bgb_call_to_action_section_meta_box_display() {
	global $post;
	...
	wp_nonce_field( 'bgb_cta_section_meta_box_nonce', 'bgb_cta_section_meta_box_nonce' );

	$editor_text = get_post_meta( $post->ID, 'cta_section_wysiwyg', true );
	wp_editor( htmlspecialchars_decode( $editor_text ), 'bgb_cta_section_wysiwyg', $settings = array( 'textarea_name' => 'call-to-action-wysiwyg' ) );
  ?>
 ...
 */
function bgb_call_to_action_section_meta_box_save($post_id) {
	if (!isset($_POST['bgb_cta_section_meta_box_nonce'])
		|| !wp_verify_nonce($_POST['bgb_cta_section_meta_box_nonce'], 'bgb_cta_section_meta_box_nonce')) {
		return;
	}
...
add_action('save_post', 'bgb_call_to_action_section_meta_box_save');
