 ...
 *
 */
function bgb_links_bar_meta_box_display() {
	global $post;
  ...
  $link_bar_2_icon = get_post_meta($post->ID, 'link_bar_2_icon', true);
	...
	wp_nonce_field( 'bgb_links_bar_meta_box_nonce', 'bgb_links_bar_meta_box_nonce' );
	?>
 ...
 *
 */
function bgb_links_bar_meta_box_save($post_id) {
	if (!isset($_POST['bgb_links_bar_meta_box_nonce'])
		|| !wp_verify_nonce($_POST['bgb_links_bar_meta_box_nonce'], 'bgb_links_bar_meta_box_nonce')) {
		return;
	}
...
add_action('save_post', 'bgb_links_bar_meta_box_save');
